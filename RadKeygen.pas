    ...
    begin
      ...
      begin
        ...
        if (FindFirst(Tmp,faAnyFile,SearchRec)=0) and
          (MessageBox(0,PChar(Format('Do you want to Delete the old slip file int %s folder',[GetAppDataPath])), PChar('Rad Studio Keygen'),MB_YESNO + MB_ICONQUESTION) = IDYES) then
        begin
          DeleteFile(PChar(Format('%s\Embarcadero\%s',[GetAppDataPath,SearchRec.Name])));
